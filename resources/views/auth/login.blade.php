...
<body class="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md z-10">
        <div class="flex items-center justify-center gap-3 mb-6 mix-blend-overlay text-white">
            <i data-lucide="crossbox" class="w-12 h-12 text-teal-300"></i>
            <h2 class="text-4xl font-extrabold tracking-tight">Q-Med</h2>
        </div>
        <h2 class="text-center text-xl font-medium tracking-wide text-teal-100 opacity-90">Sistem Antrian & Manajemen Pasien</h2>
    </div>

    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10">
        <div class="bg-white py-10 px-6 sm:rounded-2xl sm:px-10 shadow-2xl border border-slate-100">
            ...
            <form class="space-y-6" action="{{ route('login') }}" method="POST">
                ...
                <div>
                    <button type="submit"
                        class="w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white bg-teal-600 hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 transition-colors">
                        Masuk ke Dashboard
                    </button>
                </div>
            </form>
